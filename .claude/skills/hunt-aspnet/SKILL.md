---
name: hunt-aspnet
description: "Use this skill when fingerprint shows ASP.NET (X-AspNet-Version, X-Powered-By: ASP.NET, .aspx/.asmx/.svc/.ashx URLs, __VIEWSTATE in forms, ASP.NET_SessionId cookie, IIS server header), or when target is a SharePoint farm. Covers ViewState deserialization, machineKey recovery, dual-parser MAC-bypass, request-validator bypass, trace.axd/elmah.axd disclosure, load-balanced ViewState cross-node failures, SafeControl reflection enumeration, customErrors mode=Off stack-trace leaks. Only invoke this skill if there is real impact potential. Skip theoretical findings."
